...
import { RESIZE_CONSTRAINTS, TIMING } from "../utils/journalUtils";

// Combine data sources once at module level
const ALL_ITEMS = [...poetryData, ...poeticDevicesData];
...
export const ReferencePane: React.FC<ReferencePaneProps> = ({
  ...
  const activeItem = ALL_ITEMS.find((item) => item.name === activeItemName);
  ...
  const handleInsert = (text: string, id: string) => {
    if (onInsert) {
      onInsert(text);
      setInserting(id);
      setTimeout(() => setInserting(null), TIMING.INSERT_FEEDBACK_DURATION);
    }
  };
