...
const VerticalResizeHandle: React.FC<VerticalResizeHandleProps> = memo(
  ({ onResize }) => {
    ...
    return (
      ...
      >
        <div
          className={`h-px w-5 rounded-full ${isDragging ? "bg-accent" : "bg-border/40 group-hover:bg-accent/60"}`}
        />
      </div>
    );
  },
...
export const ReferencePane: React.FC<ReferencePaneProps> = ({
  ...
  useEffect(() => {
    localStorage.setItem(
      "journal_saved_list_height",
      savedListHeight.toString(),
    );
  }, [savedListHeight]);
  ...
  }, []); // allItems is stable from data imports, no need to re-compute

  const toggleCategory = useCallback(
    (category: "forms" | "meters" | "devices") => {
  ...
  return (
    ...
    >
      ...
      <div className="p-3 sm:p-4 border-b border-default flex items-center justify-between bg-bg shrink-0">
        ...
        <button
          onClick={onClose}
          className="text-muted hover:text-accent hover:bg-bg-alt/50 rounded p-1 transition-all duration-200 interactive-base interactive-scale"
        ...
        <button
          ...
          className={`flex-1 py-2 text-xs font-medium transition-all duration-200 relative interactive-base ${
            activeTab === "saved"
              ...
              : "text-muted hover:text-accent hover:bg-bg-alt/30"
          }`}
        >
          Saved
          {activeTab === "saved" && (
          ...
          >
            {pinnedItems.length > 0 ? (
              <div className="space-y-2">
                <p className="text-xs text-muted mb-3 uppercase tracking-wider font-bold">
                  Your Saved Items
                </p>
                  ...
                  <button
                    ...
                    className={`w-full text-left p-2 rounded-md border text-xs transition-all duration-200 interactive-base interactive-scale ${
                      selectedTemplate === item.name
                        ...
                        : "bg-bg border-default/50 text-muted hover:border-default hover:text-accent hover:bg-bg-alt/50 hover:shadow-sm"
                    }`}
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-medium">{item.name}</span>
                      <span className="text-[10px] opacity-60 bg-bg-alt px-1.5 py-0.5 rounded-full">
                        {item.type}
                      </span>
                    </div>
                  </button>
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {searchResults.length > 0 ? (
                  ...
                  {searchResults.map((item) => (
                    <div
                      ...
                      className={`group w-full text-left p-2 rounded-md border text-xs transition-all duration-200 cursor-pointer relative interactive-base interactive-lift ${
                        selectedTemplate === item.name
                          ? "bg-accent/10 border-accent text-default shadow-sm"
                          : "bg-bg border-default/30 text-muted hover:border-default hover:text-accent hover:bg-bg-alt/50 hover:shadow-sm"
                      }`}
                    >
                ...
                <div className="p-2 space-y-2">
                  <p className="text-[10px] text-muted uppercase tracking-wider font-bold mb-3">
                    Browse by Category
                  </p>
                      ...
                      <div className="max-h-32 overflow-y-auto border-t border-default/20">
                        {categorizedItems.forms.map((item) => (
                          <button
                            key={item.name}
                            onClick={() => onSelectTemplate(item.name)}
                            className="w-full text-left px-3 py-1.5 text-xs text-muted hover:text-accent hover:bg-accent/10 transition-all duration-200 truncate interactive-base"
                          >
                            {item.name}
                          </button>
                        ))}
                  ...
                  <div className="border border-default/30 rounded-md overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-muted">
                          ({categorizedItems.meters.length})
                        </span>
                      </div>
                      <svg
                  ...
                  <div className="border border-default/30 rounded-md overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-muted">
                          ({categorizedItems.devices.length})
                        </span>
                      </div>
                      <svg
                  ...
                  {activeItem.structure && activeItem.structure.length > 0 && (
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <h5 className="font-semibold uppercase tracking-wider text-muted text-[10px]">
                          Structure
                        </h5>
                        {onInsert && (
                          <button
